...
def make_heterodyned_likelihood_multiple_detectors(
    data_list,
    psd_list,
    response_list,
    h_function,
    ...
    raw_hp_bincenter, raw_hc_bincenter = h_function(
        f_bins_center, theta_waveform, f_ref
    )
    for i in range(num_detector):
        h_ref.append(
            response_list[i](freqs, raw_hp, raw_hc, ra, dec, gmst, ref_theta[8])
            * jnp.exp(-1j * 2 * jnp.pi * freqs * (epoch + ref_theta[5]))
        )
        h_ref_low.append(
            response_list[i](
                f_bins[:-1], raw_hp_bin, raw_hc_bin, ra, dec, gmst, ref_theta[8]
            )
            * jnp.exp(-1j * 2 * jnp.pi * f_bins[:-1] * (epoch + ref_theta[5]))
        )
        h_ref_bincenter.append(
            response_list[i](
                f_bins_center,
                raw_hp_bincenter,
                raw_hc_bincenter,
            ...
            * jnp.exp(-1j * 2 * jnp.pi * f_bins_center * (epoch + ref_theta[5]))
        )
    ...
    for i in range(num_detector):
        A0, A1, B0, B1 = compute_coefficients(
            data_list[i], h_ref[i], psd_list[i], freqs, f_bins, f_bins_center
        )
        A0_array.append(A0)
        A1_array.append(A1)
        B0_array.append(B0)
    ...
    def heterodyned_likelihood(params):
        ...
        for i in range(num_detector):
            waveform_low = response_list[i](
                f_bins[:-1], raw_hp_edge, raw_hc_edge, ra, dec, gmst, params[8]
            ) * jnp.exp(-1j * 2 * jnp.pi * f_bins[:-1] * (epoch + params[5]))
            waveform_center = response_list[i](
                f_bins_center, raw_hp_center, raw_hc_center, ra, dec, gmst, params[8]
            ) * jnp.exp(-1j * 2 * jnp.pi * f_bins_center * (epoch + params[5]))

            r0 = waveform_center / h_ref_bincenter[i]
            r1 = (waveform_low / h_ref_low[i] - r0) / (f_bins[:-1] - f_bins_center)
            match_filter_SNR = jnp.sum(
                A0_array[i] * r0.conj() + A1_array[i] * r1.conj()
            )
            optimal_SNR = jnp.sum(
                B0_array[i] * jnp.abs(r0) ** 2 + 2 * B1_array[i] * (r0 * r1.conj()).real
            )

...
V1_frequency = data['frequency'].tolist()['V1']
...
def gen_waveforms(f,theta):
    theta_waveform = theta[:9]
    ra = theta[9]
    dec = theta[10]
    hp, hc = gen_IMRPhenomD_polar(f, theta_waveform)
    return H1_response(f, hp, hc, ra, dec, theta[5], theta[8]), L1_response(f, hp, hc, ra, dec, theta[5], theta[8]), V1_response(f, hp, hc, ra, dec, theta[5], theta[8])
